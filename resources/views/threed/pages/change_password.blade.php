...
@section('content')
    <div class="w-full px-5 py-5 md:px-10">
        <div class='text-white flex border-b border-slate-200/40 pb-3'>
            ...
            <h2> ( {{ Auth::guard('threed')->user()->roles ? 'Admin' : 'User' }} ) </h2>
        </div>

        <div class='my-5 w-full grid gap-10'>

            <form action="{{ route('threed.change.password.post') }}" method="POST" autocomplete="off" id="add_form" onsubmit="return validateForm()">
