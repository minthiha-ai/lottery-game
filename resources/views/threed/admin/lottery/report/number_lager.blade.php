                ...
                @foreach ($data as $item)
                    <tr class="bg-white flex justify-between w-full bg-white border border-gray-200 rounded shadow">
                        ...
                        <td class="w-full py-2 pr-4 text-right">
                            ...
                            <a href="{{ route('threed.lottery.report.number.lager.detail',['id'=>$lotte->id,'number'=>$item->number]) }}"><i class="fa-solid fa-triangle-exclamation"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
<script>
    ...
    function updateRouteURL() {
        ...
        var currentURL = "{{ route('threed.lottery.report.number.lager.headBreak', ['id' => $lotte->id, 'price' => 'PRICE']) }}";
